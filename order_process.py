...
async def handle_order_create(
    ...
    try:
        if not payload:
            logger.error(
                "Создание ордера отменено: пустой payload для %s",
                order_info.get("mexc_symbol"),
            )
            return None
        notify_beep()
        ...
        if open_position_on_signal:
            result = await mexc_call(
                url_mode="create_order", data=payload, auth_token=mexc_auth
            )
            if result and result.get("success") and result.get("code") == 0:
                logger.info(
                    f"Ордер создан - {order_info.get('mexc_symbol')}, результат: {result}"
                )
                return result
    except Exception as exc:
...
async def prepare_payload(limit_price, side, order, leverage, max_margin, token):
    if not limit_price or limit_price <= 0:
        logger.error(
            f"[{order['mexc_symbol']}] Некорректная цена: {limit_price}. Пропускаем."
        )
        return None
    ...
    if price_scale is None or vol_scale is None:
        logger.error(
            f"[{order['mexc_symbol']}] Отсутствуют данные о точности! Невозможно рассчитать объем."
        )
        return None
    ...
    if final_volume < step:
        logger.warning(
            f"[{order['mexc_symbol']}] Рассчитанный объем < минимального шага ({final_volume} < {step}). Пропускаем."
        )
        return None

    final_price = round(limit_price, price_scale)
    logger.info(
        f"[{order['mexc_symbol']}] Рассчитанный объем ордера: {final_volume} контрактов"
    )
...
async def fetch_position_id_for_symbol(
    auth_token: str,
    symbol: str,
    want_side_open: int,
    attempts: int = 10,
        ...
        if resp and resp.get("success"):
            for r in resp.get("data", []) or []:
                if (
                    r.get("symbol") == symbol
                    and r.get("state") == 1
                    and r.get("positionType") == target_ptype
                ):
...
async def handle_order(
    token, order, redis_client: Redis, settings: dict, mexc_auth: str
):
    ...
    try:
        ...
        if not all([mexc_auth, max_margin, target_leverage]):
            logger.error(
                f"[{symbol}] Отсутствуют ключевые параметры в settings.json. Обработка прервана."
            )
            return
        ...
        if settings.get("IS_HEDGE_MODE", True):
            logger.info(
                f"[{symbol}] Режим хеджирования. Установка плеча {actual_leverage_to_set}x для LONG и SHORT."
            )
            long_res = await change_leverage(
                symbol, actual_leverage_to_set, mexc_auth, position_type=1
            )
            short_res = await change_leverage(
                symbol, actual_leverage_to_set, mexc_auth, position_type=2
            )
            ...
            if ok(long_res) and ok(short_res):
                ...
                if is_2019(long_res) or is_2019(short_res):
                    logger.warning(
                        f"[{symbol}] change_leverage вернул 2019 -> отменяю открытые ордера и повторяю."
                    )
                ...
                if ok(long_res) and ok(short_res):
                    ...
                    if is_600(long_res) and is_600(short_res):
                        logger.warning(
                            f"[{symbol}] Оба change_leverage вернули 600 в Hedge. Пробую One-way."
                        )
                        oneway_res = await change_leverage(
                            symbol, actual_leverage_to_set, mexc_auth
                        )
                        if not (oneway_res and oneway_res.get("success")):
                            logger.error(
                                f"[{symbol}] НЕ УДАЛОСЬ ИЗМЕНИТЬ ПЛЕЧО даже в One-way. LONG: {long_res}, SHORT: {short_res}, ONEWAY: {oneway_res}"
                            )
                        ...
                        logger.error(
                            f"[{symbol}] НЕ УДАЛОСЬ ИЗМЕНИТЬ ПЛЕЧО в Hedge. LONG: {long_res}, SHORT: {short_res}"
                        )
                        await redis_client.set(f"cooldown:{symbol}", 1, ex=60)
                        return
        ...
        final_leverage = leverage_res.get("data", {}).get(
            "leverage", actual_leverage_to_set
        )
        ...
        payload = await prepare_payload(
            limit_price, side, order, final_leverage, max_margin, token
        )
        if not payload:
            return
        ...
        logger.exception(
            f"Критическая ошибка в handle_order для {symbol}: {ex}", exc_info=True
        )
    finally:
...
async def finalize_order_and_setup_sl_tp(
    ...
    *,
    ...
    if not (
        main_order_res
        and main_order_res.get("success")
        and main_order_res.get("code") == 0
    ):
        logger.error(
            f"[{symbol}] НЕ УДАЛОСЬ СОЗДАТЬ ОРДЕР. Ответ: {main_order_res}. Обработка отменена."
        )
        await redis_client.set(f"cooldown:{symbol}", 1, ex=60)
        return None
    ...
    if not pos_id:
        ...
        pos_id = await fetch_position_id_for_symbol(
            mexc_auth, symbol, want_side_open=side, attempts=attempts, delay=delay
        )
    ...
    if pos_id and vol and (sl_pct or tp_pct):
        ...
        if tp_pct:
            if tp_pct:
                ...
                tp_price = (
                    adjust_price_to_tick_size(raw_tp_price, price_scale)
                    if side == 1
                    else round(limit_price * (1 - tp_pct / 100), price_scale)
                )
            tp_payload = {
                "symbol": symbol,
                "vol": vol,
        ...
        if sl_pct:
            ...
            sl_price = (
                adjust_price_to_tick_size(raw_sl_price, price_scale)
                if side == 1
                else round(limit_price * (1 + sl_pct / 100), price_scale)
            )
            sl_payload = {
                "symbol": symbol,
                "vol": vol,
        ...
        logger.warning(
            f"[{symbol}] Не удалось получить positionId для постановки SL/TP (orderId={order_id or '—'}, pos_id={pos_id})."
        )
...
async def start_process(_tokens_info, _task_status, orders_queue):
    ...
    while True:
        ...
        try:
            ...
            if settings.get("USE_COOLDOWN", True) and await redis_client.exists(
                f"cooldown:{mexc_symbol}"
            ):
                logger.info(f"Сигнал по {mexc_symbol} пропущен из-за кулдауна.")
                continue
            if token.get("is_ignored"):
                continue
            if token.get("order_in_process"):
                logger.warning(
                    f"Ордер для {mexc_symbol} уже в обработке. Сигнал пропущен."
                )
                continue
            ...
            logger.error(
                f"Критическая ошибка в цикле start_process: {ex}", exc_info=True
            )
...
async def wait_fill_and_position_id(
    auth_token: str,
    order_id: str,
    symbol: str,
    want_side_open: int,
