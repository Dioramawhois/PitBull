        ...
        try:
            ...
            async with websockets.connect(BASE_URL, open_timeout=10) as ws:
                ...
                while True:
                    ...
                    if 'base_coin_name' in message and '{' in message and '}' in message:
                        try:
                            data = json.loads(message)
                            await orders_queue.put(data)
                            logger.info(f"Получен новый сигнал: {data}")
                        except json.JSONDecodeError:
                            logger.warning(f"Не удалось декодировать JSON из сообщения: {message}")

        # Обработка конкретных ошибок веб-сокета
        except (websockets.exceptions.ConnectionClosedError, websockets.exceptions.ConnectionClosedOK) as e:
            logger.warning(f"WebSocket соединение закрыто: {e}. Переподключение через 5 секунд...")
        except asyncio.TimeoutError:
            logger.error("Тайм-аут при подключении к WebSocket. Переподключение через 5 секунд...")
        except Exception as e:
            logger.error(f"Неожиданная ошибка WebSocket: {e}. Переподключение через 5 секунд...")
